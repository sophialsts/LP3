...
public class AlocadorAssentos implements Runnable {
        ...
        try {
            if(limitadorTaxa.tryAcquire(10, TimeUnit.SECONDS)) {
                if(requisicao.getCodigoAssento().equalsIgnoreCase('ANY')) {
                    RespostaReserva resposta = alocarQualquerAssento();
                    enviarResposta(resposta);
                }
                else {
                    RespostaReserva resposta = alocarAssentoEspecifico(requisicao, requisicao.getCodigoAssento());
                    enviarResposta(resposta);
                }
            }
        }
    ...
    private RespostaReserva alocarQualquerAssento(RequisicaoReserva requisicao) {
        ...
        for(String assento:mapaAssentos.getTodosCodigosAssentos()) {
            if (mapaAssentos.getAssento() == null) {
                return RespostaReserva.falha("ASSENTO_INVALIDO", "Assento " + assento + " não existe");
            }

            try {

                if(!mapaAssentos.getAssento(assento).getTrava().tryLock(5, TimeUnit.SECONDS)) {
                    return RespostaReserva.falha("TIMEOUT", "Timeout ao acessar assento");
                }

                try {
                    if(assento.equalsIgnoreCase(" X")) {
                        return RespostaReserva.falha("TIMEOUT", "Timeout ao acessar assento");
                    }


                    if(mapaAssentos.getAssento(assento).getFlagOcupado().compareAndSed(false,true)) {
                            String codigo = gerarCodigoReserva();
                            mapaAssentos.getAssento(assento).ocupar();
                            mapaAssentos.decrementarAssentosLivres();
                            registrarReserva(requisicao, assento, codigo);
                            return RespostaReserva.sucesso(assento, codigo);
                    }
                    else {
                        continue;
                    }
                    //faltou fzr lógica pra saber que tá no fim 
                }
                finally {
                    mapaAssentos.getAssento(assento).unlock();
                }
            }
            catch(InterruptedException e) {
                Thread.currentThread().interrupt();
