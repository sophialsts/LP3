    ...
    private String tratarReserva(String[] partes) {
        ...
        try {
            if(partes.length >= 4) {
                ...
                String nome = (partes[1] + partes[2].toUpperCase());
                String categoria = (partes[3]).toUpperCase();
                String assento = (partes[4]).toUpperCase();

                String NOME = String.valueOf(nome);
                int CATEGORIA = Integer.valueOf(categoria);
                String ASSENTO = String.valueOf(assento);


                RequisicaoReserva novaRequisicao = new RequisicaoReserva(NOME, CATEGORIA, ASSENTO);
                if(filaRequisicoes.offer(novaRequisicao) == null) {
                    System.out.println("FAIL ");
                }
                novaRequisicao.getFilaResposta().getSucesso();
